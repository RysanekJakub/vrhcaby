...
class Hernipole:
    ...
    def tah(self, id_hrace, hod_kostkou):
        ...
        if id_hrace == 1:
            ...
            # po presunu nezbyde kamen na startu
            if klic[2] == "pridani" and len(self.spikes[start]["kameny"]) == 1:
                presouvany_kamen = self.spikes[start]["kameny"][-1]
                presouvany_kamen.pamet.append(end)
                del self.spikes[start]["kameny"][-1]
                self.spikes[start]["barva"] = None
                
                self.spikes[end]["kameny"].append(presouvany_kamen)
                
            # po presunu zbyde kamen na startu
            if klic[2] == "pridani" and len(self.spikes[start]["kameny"]) > 1:
                presouvany_kamen = self.spikes[start]["kameny"][-1]
                presouvany_kamen.pamet.append(end)
                del self.spikes[start]["kameny"][-1]
        ...
        if id_hrace == 2:
            ...
            mozne_tahy = []
            for i in range(len(self.spikes)):
                if self.spikes[i]["barva"] == "cerna" and i+hod_kostkou < 23:               # spike s barvou hrace
                    ...
                    # spike na ktery presouvame ma barvu protivnika a je jen jeden kamen
                    if self.spikes[i+hod_kostkou]["barva"] == "bila" and len(self.spikes[i+hod_kostkou]["kameny"]) == 1:
                        mozne_tahy.append((i, i+hod_kostkou, "vyhozeni"))
                        
                    # spike na ktery presouvame ma barvu hrace
                    if self.spikes[i+hod_kostkou]["barva"] == "cerna" and len(self.spikes[i+hod_kostkou]["kameny"]) > 0:
                        mozne_tahy.append((i, i+hod_kostkou, "pridani"))
            ...
            if klic[2] == "prazdno" and len(self.spikes[start]["kameny"]) == 1:
                ...
                self.spikes[end]["barva"] = "cerna"                                    # nastaveni nove barvy
        ...
        if id_hrace == 3:
            ...
            # priorita 1.) vyhozeni 2.) pridani aby nebyl vyhozen protivnikem 3.) pridani
            for start, end, druh in mozne_tahy:
                if druh == "vyhozeni":
                    klic = (mozne_tahy[0], mozne_tahy[1], mozne_tahy[2])
                    break
                if druh == "pridani":
                    klic = (mozne_tahy[0], mozne_tahy[1], mozne_tahy[2])
                    break
